...
def terrain_levels_vel(
    ...
    move_up = distance > terrain.cfg.terrain_generator.size[0] / (2 * 2.2)
    # robots that walked less than half of their required distance go to simpler terrains
    move_down = distance < torch.norm(command[env_ids, :2], dim=1) * env.max_episode_length_s * (0.5 / 2.2)
    move_down *= ~move_up
    ...
    print_stats = True
    if print_stats and env.common_step_counter % 100 == 0:
        print(f"Avg distance walked: {torch.mean(distance)}")
        print(f"Max distance walked: {torch.max(distance)}")
        print(f"Threshold to move up: {terrain.cfg.terrain_generator.size[0] / (2 * 2.2)}")
        print(f"Threshold to move down: {torch.norm(command[env_ids, :2], dim=1) * env.max_episode_length_s * (0.5 / 2.2)}")
        print(f"Number of robots that moved up: {torch.sum(move_up)}")
        print(f"Number of robots that moved down: {torch.sum(move_down)}")
