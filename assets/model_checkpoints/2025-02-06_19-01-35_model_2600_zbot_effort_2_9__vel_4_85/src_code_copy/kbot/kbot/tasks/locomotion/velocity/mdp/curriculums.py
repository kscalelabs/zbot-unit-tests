...
def modify_push_force(
    ...
    """
    custom_interval = 200
    if env.common_step_counter % custom_interval == 0:
        print(f"in push force function, env.common_step_counter: {env.common_step_counter}, starting_step: {starting_step}, interval: {interval}")
    try:
    ...
    # print(f"in push force function, curr_setting: {curr_setting}")
    if env.common_step_counter < starting_step:
        return curr_setting
    if env.common_step_counter % interval == 0:
        # print(f"in push force function, env.common_step_counter ({env.common_step_counter}) % interval ({interval}) == 0: {env.common_step_counter % interval == 0}")
        
        if torch.sum(env.termination_manager._term_dones["base_contact"]) < torch.sum(env.termination_manager._term_dones["time_out"]) * 2:
            print(f"in pursh force, increasing push force")
            print(f"in push force function, torch.sum(env.termination_manager._term_dones['base_contact']) ({torch.sum(env.termination_manager._term_dones['base_contact'])}) < torch.sum(env.termination_manager._term_dones['time_out']) ({torch.sum(env.termination_manager._term_dones['time_out'])}) * 2: {torch.sum(env.termination_manager._term_dones['base_contact']) < torch.sum(env.termination_manager._term_dones['time_out']) * 2}")
            # obtain term settings
            term_cfg = env.event_manager.get_term_cfg('push_robot')
            # update term settings
            curr_setting = term_cfg.params['velocity_range']['x'][1]
            curr_setting = np.clip(curr_setting * 1.5, 0.2, max_velocity[0])
            term_cfg.params['velocity_range']['x'] = (-curr_setting, curr_setting)
            curr_setting = term_cfg.params['velocity_range']['y'][1]
            curr_setting = np.clip(curr_setting * 1.5, 0.2, max_velocity[1])
            term_cfg.params['velocity_range']['y'] = (-curr_setting, curr_setting)
            env.event_manager.set_term_cfg('push_robot', term_cfg)
        

        if torch.sum(env.termination_manager._term_dones["base_contact"]) > torch.sum(env.termination_manager._term_dones["time_out"]) / 2:
            print(f"in pursh force, decreasing push force")
            print(f"in push force function, torch.sum(env.termination_manager._term_dones['base_contact']) ({torch.sum(env.termination_manager._term_dones['base_contact'])}) > torch.sum(env.termination_manager._term_dones['time_out']) ({torch.sum(env.termination_manager._term_dones['time_out'])}) / 2: {torch.sum(env.termination_manager._term_dones['base_contact']) > torch.sum(env.termination_manager._term_dones['time_out']) / 2}")
            # obtain term settings
            term_cfg = env.event_manager.get_term_cfg('push_robot')
            # update term settings
            curr_setting = term_cfg.params['velocity_range']['x'][1]
            curr_setting = np.clip(curr_setting - 0.2, 0.1, max_velocity[0])
            term_cfg.params['velocity_range']['x'] = (-curr_setting, curr_setting)
            curr_setting = term_cfg.params['velocity_range']['y'][1]
            curr_setting = np.clip(curr_setting - 0.2, 0.1, max_velocity[1])
            term_cfg.params['velocity_range']['y'] = (-curr_setting, curr_setting)
            env.event_manager.set_term_cfg('push_robot', term_cfg)
    # print(f"in push force function, curr_setting: {curr_setting if curr_setting is not None else 'None'}")
