...
class EventsCfg:
    ...
    #     mode="startup",
    #     params={
    #         "asset_cfg": SceneEntityCfg("robot", body_names="body1_part"),
    #         "mass_distribution_params": (-5.0, 5.0),
    #         "operation": "add",
    #     },
    # )

    # # scale actuator params 
    ...
    # scale_all_joint_stiffness = EventTerm(
    ...
    # scale_all_joint_damping = EventTerm(
    #     func=mdp.randomize_actuator_gains,
    #     mode="startup",
    #     params={
    #         "asset_cfg": SceneEntityCfg("robot", joint_names=[".*"]),
    ...
    # no random base external force torque
    base_external_force_torque = EventTerm(
        func=mdp.apply_external_force_torque,
        mode="reset",
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="base"),
            "force_range": (0.0, 0.0),   # Reasonable force range for humanoid
            "torque_range": (0.0, 0.0),  # Reasonable torque range for humanoid
        },
    )

    # no random reset base 
    reset_base = EventTerm(
        ...
        params={
            ...
            "velocity_range": {
                "x": (0.0, 0.0),     # Initial velocity randomization
                "y": (0.0, 0.0),     # Initial velocity randomization
                "z": (0.0, 0.0),     # Small vertical velocity
                "roll": (0.0, 0.0),  # Initial angular velocity
                "pitch": (0.0, 0.0), # Initial angular velocity
                "yaw": (0.0, 0.0),   # Initial angular velocity
            },
        },
    )

    # no random reset robot joints
    ...
    # this only runs once on inital startup, meant to simulate calibration error
    # scale_all_joint_default_pos = EventTerm(
    #     func=mdp.randomize_joint_default_pos,
    #     mode="startup",
    #     params={
    #         "asset_cfg": SceneEntityCfg("robot", joint_names=[".*"]),
    #         "pos_distribution_params": (0.5, 1.5),
    #         "operation": "scale",
    #     },
    # )

    # # scale random stuff happeninig on reset 
    # # reset
    # base_external_force_torque = EventTerm(
    #     func=mdp.apply_external_force_torque,
    #     mode="reset",
    #     params={
    #         "asset_cfg": SceneEntityCfg("robot", body_names="base"),
    #         "force_range": (-20.0, 20.0),   # Reasonable force range for humanoid
    #         "torque_range": (-10.0, 10.0),  # Reasonable torque range for humanoid
    #     },
    # )
    ...
    #         "pose_range": {"x": (-0.5, 0.5), "y": (-0.5, 0.5), "yaw": (-3.14, 3.14)},
    #         "velocity_range": {
    #             "x": (-0.5, 0.5),     # Initial velocity randomization
    #             "y": (-0.5, 0.5),     # Initial velocity randomization
    #             "z": (0.0, 0.5),     # Small vertical velocity
    #             "roll": (-0.5, 0.5),  # Initial angular velocity
    #             "pitch": (-0.5, 0.5), # Initial angular velocity
    #             "yaw": (-0.5, 0.5),   # Initial angular velocity
    #         },
    #     },
    # )
    ...
    push_robot = EventTerm(
        ...
        # params={"velocity_range": {"x": (-5.0, 5.0), "y": (-5.0, 5.0)}},
...
class RewardsCfg:
    ...
    joint_torques_l2 = RewTerm(func=mdp.joint_torques_l2, weight=-1.5e-7, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_hip_.*", ".*_knee_.*", ".*_ankle_.*"])})
    dof_acc_l2 = RewTerm(func=mdp.joint_acc_l2, weight=-1.25e-7, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_hip_.*", ".*_knee_.*",])})
    ...
    dof_pos_limits = RewTerm(
        ...
        weight=-1.0,
        params={"asset_cfg": SceneEntityCfg("robot", joint_names=["left_ankle_02", "right_ankle_02"])},
    )
    ...
    #     weight=-1.0,
    #     params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=[".*leg2_shell.*"]), "threshold": 1.0},
    # )
